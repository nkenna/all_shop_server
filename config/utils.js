...
generateAccessToken: (username) => {
     return jwt.sign(username, process.env.TOKEN_SECRET, { expiresIn: '3000d' });
},

generateAdminAccessToken: (data) => {
    return jwt.sign(data, process.env.TOKEN_SECRET, { expiresIn: '3000d' });
},
...
calculateCommission: (amount, percent) => {
    // commission is usually 10%
    var perc = percent;
    var charge = ((perc/100) * amount);    
    return charge;
